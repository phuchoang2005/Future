...
export function ProjectDashboardPage() {
  ...
  const visibleProjects = projects.filter((project) => `${project.projectName} ${project.description}`.toLowerCase().includes(query.toLowerCase()) && (filter === "ALL" || project.latestJobStatus === filter));
  ...
  return (
    <Page>
      ...
      {visibleProjects.length === 0 ? <EmptyState icon={<FolderKanban />} title="No authorized projects" message="No projects match the current filters." /> : <ProjectTable projects={visibleProjects} />}
    </Page>
  );
}
